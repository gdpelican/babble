...
export default Discourse.Route.extend({
  ...
  setupController: function(controller) {
    ...
    Group.findAll().then((groupsResponse) => {
      ...
      let groups = _.reject(groupsResponse.map((g) => { g.automatic = false; return g; }), (g) => {
        return g.id == 0
      })

      if (id === 'new') {
        controller.setProperties({ model: Topic.create(), available: groups, selected: [] })
      } else {
        ajax(`/babble/topics/${id}.json`).then((data) => {
          ...
          ajax(`/babble/topics/${id}/groups.json`).then((response) => {
            ...
            controller.setProperties({ model: topic, available: groups, selected: selected })
          })
        })
      }
    })
